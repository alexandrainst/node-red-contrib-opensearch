module.exports = function (RED) {
    ...
    function Tail(n) {
        ...
        if (n.filter) {
            const data = U.prepData(node, {});
            const filter = M.render(n.filter, data);
        ...
        if (node.conf.composition !== '') { params._source_include = node.conf.composition.split(','); }
        ...
        this.on('ticktock', async function () {
            ...
            SCROLL: for await (const res of scrollSearch) {
                ...
                for (const d in hits) {
                    if (node.seenDocs[hits[d]._id]) { continue; }
                    count++;
